<script>
	// import FirebaseEraser from './../components/FirebaseEraser.svelte';
  // *- IMPORTACIONES
  ...
  // *- IMPORTA LA BASE DE DATOS DE FIREBASE
      ...
      // Delete Binnacle de prueba     ************** borrar de firebase info  *********************
            // let binnacles = [];
  ...
  // *- FILTROS
      // Contacto seleccionado ""selecContact""
            const contactSelect = (currentContact, binnacles) => {
              ...
              contact.tagsProperty = currentContact.tagsProperty;
              contact.locaProperty = currentContact.locaProperty;
              contact.sendedProperties = currentContact.sendedProperties;
      ...
      // Enviar a los checkbox seleccionados            
            const checkedT = () => {
              if(commInpuyBinnacle) {
                ...
                window.open(link)
                commBinnacle= (`Se envió el mensaje: ${commInpuyBinnacle} a ${contact.name} al ${contact.telephon}`);
                binnacle = {"date": Date.now(), "comment": commBinnacle}
                commInpuyBinnacle = "";
              } else {
                  if(checkedProperty.length === 0 ){
                    ...
                    checkedProperty.forEach((checkedProperty) => {                    
                      let link = (`https://api.whatsapp.com/send?phone=52${contact.telephon}&text=${checkedProperty.urlProp}`)
                      window.open(link)
                      commBinnacle= (`Se envió ${checkedProperty.nameProperty} a ${contact.name} ${contact.lastname} al ${contact.telephon}`);
                      editingItem = "sendProperty";
                      claEB = checkedProperty.claveEB
                      updateContact(contact, claEB)
                      binnacle = {"date": Date.now(), "comment": commBinnacle}
                      saveBinnacle(binnacle);
                    });               
                  } else {
